...
"""`Game-it`em recommender system environment.

A gaming user interacts with a mobile game by clicking (and buying)
zero or more game items (such as weapons or alien artifacts),
recommended to her by our RL agent.
...
class GamingUserModel(user.AbstractUserModel):
  ...
  def __init__(self,
    ...
    super().__init__(
        response_model_ctor,
        LTSStaticUserSampler(user_ctor=user_state_ctor, seed=seed),
        slate_size)
...
class GamingUserResponse(user.AbstractResponse):
  """Class to represent a gaming user's response to a game item.

  Attributes:
    clicked: boolean indicating whether the item was clicked (and bought) or not.
  """
  ...
  def response_space(cls):
    return spaces.Dict({
        # Whether the user clicked (and bought) the item or not.
        "click_and_buy": spaces.Discrete(2),
    })


class GameItemToBuy(document.AbstractDocument):
  """Class to represent a in-game buyable item.

  Attributes:
    fantasy_geekiness: Real number in [0,1] representing the degree of this item's
        fantasy geekiness.
    scifi_geekiness: Real number in [0,1] representing the degree of this item's
        SciFi geekiness.
  """
...
class GameItemSampler(document.AbstractDocumentSampler):
  """Class to sample GameItems.

    Args:
        game_item_ctor: A class/constructor for the type of documents that will be sampled
          by this sampler.
  """

  def __init__(self, game_item_ctor: Type[document.AbstractDocument] = GameItemToBuy, **kwargs):
    super().__init__(game_item_ctor, **kwargs)
    self._doc_count = 0

  def sample_document(self):
    doc_features = {}
    doc_features["doc_id"] = self._doc_count
    doc_features["fantasy_geekiness"] = self._rng.random_sample()
    doc_features["scifi_geekiness"] = self._rng.random_sample()
    self._doc_count += 1
...
def create_environment(env_config):
  ...
  document_sampler = GameItemSampler()
...
InGameItemRecoEnvSimulator = make_recsim_env(
    user_model=GamingUserModel,
    document_sampler=
)
